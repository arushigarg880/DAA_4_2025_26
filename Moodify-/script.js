...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const toSignupText = document.getElementById('to-signup');
    ...
    if (toSignupText) toSignupText.addEventListener('click', () => openModal('signup'));
    if (toLoginText) toLoginText.addEventListener('click', () => openModal('login'));
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                errorEl.style.color = '#1ed760';
                errorEl.innerText = 'Logging in...';
                
                // Example Fetch Call (Uncomment to use with real API):
                ...
                errorEl.innerText = 'Login successful! (Simulated)';
    ...
    if (signupForm) {
        signupForm.addEventListener('submit', async (e) => {
            ...
            if (!name) {
                errorEl.innerText = 'Profile name is required.';
                return;
            }
            if (!validateEmail(email)) {
                errorEl.innerText = 'Please enter a valid email address.';
            ...
            try {
                ...
                errorEl.innerText = 'Account created successfully! (Simulated)';
                setTimeout(closeAuthModal, 1000);
            } catch (err) {
                errorEl.style.color = '#ff6b6b';
                errorEl.innerText = 'Failed to create account.';
            }
        });
    }
